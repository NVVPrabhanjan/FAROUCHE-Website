...
export default function FilteredGallery() {
  ...
  return (
    <div className="min-h-screen bg-black text-white overflow-x-hidden pt-12">
      ...
      <div className="relative min-h-screen pt-20 pb-10 bg-gradient-to-b from-amber-900/30 via-amber-950/15 to-black">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="flex justify-center flex-wrap gap-4 mb-12">
            <Link
              href="/gallery"
              className="px-8 py-3 rounded-full text-lg font-medium transition-all duration-300 bg-gradient-to-r from-amber-900/70 to-amber-800/70 text-amber-200 hover:from-amber-800 hover:to-amber-700 hover:shadow-md"
            >
              All Events
            </Link>
            {events.map((event) => (
              <Link
                ...
                className={`
                  ...
                  ${event === currentEvent
                    ? "bg-gradient-to-r from-amber-600 to-amber-500 text-white shadow-lg transform scale-105" 
                    : "bg-gradient-to-r from-amber-900/70 to-amber-800/70 text-amber-200 hover:from-amber-800 hover:to-amber-700 hover:shadow-md"
                  }
                `}
              >
          ...
          {!loading && images.length > 0 && (
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
              {images.map((img, idx) => (
                <div key={idx} className="group relative overflow-hidden rounded-lg transition-all duration-300 transform hover:scale-105 hover:shadow-lg shadow-md shadow-amber-900/30">
                  <img
                    src={img}
          ...
          {loading && (
            <div className="text-center py-20">
              <p className="text-xl text-amber-300">Loading gallery images...</p>
              <div className="mt-4 w-12 h-12 rounded-full border-4 border-t-amber-500 border-r-amber-500 border-b-transparent border-l-transparent animate-spin mx-auto"></div>
            </div>
          )}
          ...
          {!loading && images.length === 0 && !message && (
            <div className="text-center py-20">
              <p className="text-xl text-amber-300">No images available for {currentEvent}</p>
              <Link 
                href="/gallery" 
                className="mt-6 inline-block px-6 py-2 rounded-full bg-gradient-to-r from-amber-600 to-amber-500 text-white font-medium hover:shadow-lg transition-all duration-300"
              >
